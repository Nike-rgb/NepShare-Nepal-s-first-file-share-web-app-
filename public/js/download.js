...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _chat_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./chat.js */ "./resources/js/chat.js");
...
const uuid = document.querySelector('.uuid').innerText;
_chat_js__WEBPACK_IMPORTED_MODULE_0__.socket.emit('join', uuid); //joining the same room as sender
_chat_js__WEBPACK_IMPORTED_MODULE_0__.socket.emit('active', uuid);//send active signal to the sender
document.querySelector('.chat-btn').style.display = "block"; //displaying the chat button

//for more incoming files
_chat_js__WEBPACK_IMPORTED_MODULE_0__.socket.on('moreFiles', number => {
  moreFiles.style.display = "block";
  moreFiles.children[1].innerText = `${number} incoming files...`;
});

//once more files sending finishes
_chat_js__WEBPACK_IMPORTED_MODULE_0__.socket.on('moreReceived', () => {
  moreFiles.style.display = ""; //cleanups
  //query the database and fetch the uploaded files
  axios__WEBPACK_IMPORTED_MODULE_1___default().post('/files/fetch-more-files', {uuid}).then(res => {
